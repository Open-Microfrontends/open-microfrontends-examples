...
package org.openmicrofrontends.demo._generated;
...
public class OpenMicrofrontendHostIntegrations {
    ...
    private static final String OM_BASE_PATH = "/__om__";
...
public static class OpenMicrofrontendsExampleAPIProxyWithSecurityHostIntegrationFilter implements Filter {
    ...
    public interface MicrofrontendBaseSetup {
        ...
        default Map<String, String> getHostContext() {
            return null;
                    ...
                    *  Schemes: [{"ApiKeyAuth":[]}]
                    */
                    Map<String, String> getApiProxyRequestBffGetSecurityHeaders(HttpServletRequest req);
                    ...
                    */
                    String getApiProxyChuckNorrisJokeUrl();
    ...
    private static final String ATTR_EXTRA_HEADERS = OpenMicrofrontendsExampleAPIProxyWithSecurityHostIntegrationFilter.class + ".extraHeaders";

    private final MicrofrontendBaseSetup baseSetup;
    private final HttpClient httpClient;
    private final InternalProxyServlet proxyServlet;

    public OpenMicrofrontendsExampleAPIProxyWithSecurityHostIntegrationFilter(MicrofrontendBaseSetup baseSetup) {
    ...
    public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain) throws IOException, ServletException {
        ...
        if (!url.startsWith(OM_BASE_PATH + "/OpenMicrofrontendsExampleAPIProxyWithSecurity")) {
            chain.doFilter(req, res);
            return;
        }

        url = url.replace(OM_BASE_PATH + "/OpenMicrofrontendsExampleAPIProxyWithSecurity", "");
        logger.debug("[OpenMicrofrontends] Processing request for Microfrontend OpenMicrofrontendsExampleAPIProxyWithSecurity: {}", url);
        ...
        if (httpReq.getMethod().equals("GET") && url.equals(OM_SETUP_SUB_PATH)) {
            try {
                ...
                logger.error("[OpenMicrofrontends] Error while processing config request for Microfrontend OpenMicrofrontendsExampleAPIProxyWithSecurity", e);
        ...
        if (httpReq.getMethod().equals("GET") && url.startsWith(OM_ASSETS_SUB_PATH)) {
            var targetAssetPath = toFullUrl(baseSetup.getMicrofrontendBaseUrl(), "/public", url.substring(OM_ASSETS_SUB_PATH.length()));
            try {
                proxyServlet.proxy(httpReq, httpRes, targetAssetPath, null);
                return;
            } catch (Exception e) {
                logger.error("[OpenMicrofrontends] Error while fetching asset " + targetAssetPath + " for Microfrontend OpenMicrofrontendsExampleAPIProxyWithSecurity", e);
                if (!httpRes.isCommitted()) {
                    httpRes.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
                }
                return;
            ...
            if (url.startsWith(OM_PROXIES_SUB_PATH)) {
                var proxyUrl = url.substring(OM_PROXIES_SUB_PATH.length());
                    ...
                    if (proxyUrl.startsWith("/bff")) {
                        proxyUrl = proxyUrl.substring("/bff".length());
                                                    var securityHeaders = baseSetup.getApiProxyRequestBffGetSecurityHeaders(httpReq);
                        ...
                        try {
                            ...
                            logger.error("[OpenMicrofrontends] Error while forwarding call to proxy 'bff' for Microfrontend OpenMicrofrontendsExampleAPIProxyWithSecurity. Target:" + httpReq.getMethod() + " " + proxyTargetPath, e);
                            if (!httpRes.isCommitted()) {
                                httpRes.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
                            }
                            return;
                    ...
                    if (proxyUrl.startsWith("/chuckNorrisJoke")) {
                        proxyUrl = proxyUrl.substring("/chuckNorrisJoke".length());
                                                    Map<String, String> securityHeaders = null;
                                                                            var proxyTargetPath = toFullUrl(baseSetup.getApiProxyChuckNorrisJokeUrl(), proxyUrl);
                        
                        try {
                            proxyServlet.proxy(httpReq, httpRes, proxyTargetPath, securityHeaders);
                            return;
                        } catch (Exception e) {
                            logger.error("[OpenMicrofrontends] Error while forwarding call to proxy 'chuckNorrisJoke' for Microfrontend OpenMicrofrontendsExampleAPIProxyWithSecurity. Target:" + httpReq.getMethod() + " " + proxyTargetPath, e);
                            if (!httpRes.isCommitted()) {
                                httpRes.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
                            }
                            return;
    ...
    private String getBuildTimestampOrVersion() throws IOException, InterruptedException {
            ...
            try {
                var res = fetchWithExtraHeaders("/package.json", baseSetup.getMicrofrontendBaseUrl(), Map.of());
    ...
    private class InternalProxyServlet extends ProxyServlet {
        ...
        protected void service(HttpServletRequest req, HttpServletResponse resp) throws IOException {
            try {
                ...
                logger.error("[OpenMicrofrontends] Error during proxying for Microfrontend OpenMicrofrontendsExampleAPIProxyWithSecurity. Request URL: {}", req.getRequestURL(), e);
                if (!resp.isCommitted()) {
